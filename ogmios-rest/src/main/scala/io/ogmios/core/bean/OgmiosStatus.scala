 ...
 */
case class OpCompleted(val state: String, val msg : String = "") extends OgmiosStatus
/**
 * Use when the operation failed
 */
case class OpFailed(val state: String, val msg : String) extends OgmiosStatus
/**
 * Use when the operation is successful and return a bean.
 * the bean is set in the value attribute
 */
case class OpResult[T](val state: String, val value: T, val msg : String="") extends OgmiosStatus
